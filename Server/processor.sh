...
testConn=`ssh -4 -p $sshPort -o StrictHostKeyChecking=no -l bluesky -i /usr/local/bin/BlueSky/Server/blueskyd localhost "/usr/bin/defaults read /var/bluesky/settings serial"`
testExit=$?
if [ $testExit -eq 0 ]; then
  if [ "$testConn" == "$serialNum" ]; then
	...
	testConn2=`ssh -4 -p $sshPort -o StrictHostKeyChecking=no -l bluesky -i /usr/local/bin/BlueSky/Server/blueskyd localhost "/usr/libexec/PlistBuddy -c 'Print serial' /var/bluesky/settings.plist"`
	testExit2=$?
