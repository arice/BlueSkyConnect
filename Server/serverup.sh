...
for serialNum in $alertList; do
	...
	if [ ${lastConn:-0} -lt $checkThresh ]; then
	  # its been quiet for more than 10 min, might be down
		...
		testSN=`ssh -4 -p $sshPort -o ConnectTimeout=5 -o ConnectionAttempts=5 -o StrictHostKeyChecking=no -l bluesky -i /usr/local/bin/BlueSky/Server/blueskyd localhost "/usr/bin/defaults read /var/bluesky/settings serial"`
		testExit=$?
